...
Fix for NumPy>=2.x / Py3.14:
- NumPy no longer exposes np.RankWarning; silence polyfit warning by message.
...
def best_degree_for_fixed_window(
    ...
    for deg in fit_degrees:
        ...
        if best is None:
            best = cand
            if fits:  # degrees ascending: first fit wins
                break
            continue
...
def segment_piecewise_greedy(x: np.ndarray, y: np.ndarray, eps: float, fit_degrees: List[int], offset: int, kind: str) -> List[Segment]:
    """
    Greedy PWPR segmenter on sliced arrays x,y. Offsets indices by 'offset'.
...
def build_symmetric_extrema_windows(n: int, extrema_idx: np.ndarray) -> List[Tuple[int, int, int]]:
    ...
    Returns (s, e_excl, k). May overlap; caller resolves.
    """
...
def resolve_nonoverlapping(windows: List[Tuple[int, int, int]]) -> List[Tuple[int, int, int]]:
    """
    Make windows non-overlapping by trimming the later window's start to the cursor.
    Trimming can break perfect symmetry but keeps the "extremum near center" idea.
    Windows <3 points after trimming are dropped.
    """
...
def segment_centered_sym_extrema_full(
    ...
    Full-domain segmentation:
      - symmetric extrema windows
      - resolve overlaps (trim)
      - fill gaps with greedy segmentation
      - choose best degree for each extrema window
    """
    n = len(x)
    fit_degrees = sorted(fit_degrees) or [1]
