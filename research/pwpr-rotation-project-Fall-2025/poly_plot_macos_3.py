...
Key update (per your note “deg1 dominates”):
- Segment degree selection is no longer “lowest degree that fits eps”.
- Instead, for each fixed window we choose the degree that MINIMIZES max error
  (with a tiny complexity penalty you can tune), so higher degrees can win when
  they genuinely fit better—especially around extrema windows.

Also:
- Forces an interactive backend + enables Matplotlib toolbar buttons.

Run:
  python3 poly_plot_macos.py
"""
...
# Degree choice tuning:
# score = max_abs_error + DEG_PENALTY * degree
# Make DEG_PENALTY smaller to let higher degrees win more often.
...
def best_degree_for_fixed_window(
    ...
    """
    Choose degree for fixed window [start,end_excl):
      - Prefer any degree that fits eps; among those choose minimal SCORE.
      - If none fit, choose minimal SCORE anyway.
    SCORE = max_abs_error + penalty*deg
    """
    length = end_excl - start
    fit_degrees = sorted(fit_degrees) or [1]
    penalty = DEG_PENALTY_EXTREMA if kind == "extrema" else DEG_PENALTY_GAP

    best_fit = None   # (score, err, deg, coef, mu, scale)
    best_any = None   # (score, err, deg, coef, mu, scale)
...
def resolve_nonoverlapping(windows: List[Tuple[int, int, int]]) -> List[Tuple[int, int, int]]:
    """
    Make windows non-overlapping by trimming later window start to cursor.
    (This can break perfect symmetry if extrema are very close.)
    """
...
def segment_centered_sym_extrema_full(
    ...
    Full-domain segmentation:
      ...
      - choose degree for extrema windows via best_degree_for_fixed_window
    """
    n = len(x)
    fit_degrees = sorted(fit_degrees) or [1]
...
class PolyPlotApp:
    def __init__(self):
        ...
        ax_fit_label.text(0.0, -0.55, f"(Extrema penalty={DEG_PENALTY_EXTREMA}, Gap penalty={DEG_PENALTY_GAP})", family="monospace", fontsize=9)
    ...
    def redraw(self):
        ...
        lines.append(f"Degree score: err + penalty*deg  (extrema penalty={DEG_PENALTY_EXTREMA}, gap penalty={DEG_PENALTY_GAP})")
